...
from madrid_instruments_service.operation.operations import Operation
...
class ProgramOperations:
    ...
    def new(self) -> ProgramId:
        self.counter += 1
        self.id = ProgramId(f"MadridProgramId{self.counter}")
        self.operations[self.id] = []
        return self.id

    def add_operation(self, operation: Operation):
        if not isinstance(operation, Operation):
            raise TypeError(f"Operation type not recognized {operation.__class__.__name__}")
